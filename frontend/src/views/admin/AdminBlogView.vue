    ...
    <div v-if="showAdd" class="row mb-4">
      <div class="col-12">
        <div class="card border-0 shadow-sm">
          <div class="card-body" style="padding-bottom: 80px; position: relative;">
            ...
            <div class="row g-3">
              ...
              <div class="col-12">
                ...
                <quill-editor
                  ...
                  style="max-height: 220px; background: white;"
                />
              </div>
            </div>
              ...
              <div class="col-12 col-md-6 col-lg-4" v-for="blog in blogPreviews" :key="blog.id">
                <div class="card h-100 shadow-sm">
                  ...
                  <div class="card-body d-flex flex-column">
                    ...
                    <p class="mb-2 text-truncate" style="max-height:3.6em; overflow:hidden">{{ blog.preview }}</p>
  ...
  methods: {
    ...
    async submitBlog() {
      try {
        ...
        const response = await axios.post(`${API_URL}/api/blogs`, payload, { headers: { 'Content-Type': 'application/json' } });
        alert('Blog posted successfully!');
        this.resetForm();
        await this.fetchBlogs();
      } catch (err) { console.error(err); alert('Failed to post blog') }
    },
    async fetchBlogs() { try { const res = await axios.get(`${API_URL}/api/blogs`); this.blogs = res.data || []; } catch (err) { console.error(err) } },
    ...
    async updateBlog() { try { const payload = { title:this.title.trim(), author:this.author.trim(), location:this.location.trim(), imageUrl:this.imageUrl?.trim()||null, content:this.content }; await axios.put(`${API_URL}/api/blogs/${this.editingBlog.id}`, payload); alert('Blog updated'); this.resetForm(); await this.fetchBlogs(); } catch (err) { console.error(err); alert('Update failed') } },
    async deleteBlog(id) { if(!confirm('Delete this blog?')) return; try { await axios.delete(`${API_URL}/api/blogs/${id}`); alert('Deleted'); await this.fetchBlogs(); } catch (err) { console.error(err); alert('Delete failed') } },
...
<style scoped>
  ...
  /* Blog card specific styles to remove gap */
  .card.h-100 .card-body {
    padding-bottom: 1rem !important;
  }

  .card.h-100 .card-body .mt-auto.pt-2 {
